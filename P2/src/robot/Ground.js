...
class Ground extends THREE.Object3D {
  ...
  intersectOvos (b1, b2) {
    ...
    vectorBetweenOvos.subVectors (new THREE.Vector3 (b1.position.x, b1.position.y, b1.position.z),
                                   new THREE.Vector3 (b2.position.x, b2.position.y, b2.position.z));
    return (vectorBetweenOvos.length() <= (((b1.ovoRadius*2)+(b2.ovoRadius*2))/2));
  }

  updateColisionOvos () {
  	//var k = this.ovos.children.length-1;
  	var k = 0;
    if (k=>0) {
      for (var i = k; i < this.ovos.children.length; i++) {
        for (var j = 0; j < i; j++) {
          if (this.intersectOvos (this.ovos.children[j], this.ovos.children[i])) {
          	/*
          	this.ovos.children[i].material.color.setHex(0xf0f0f0);
  			this.ovos.children[j].material.color.setHex(0xf0f0f0);
  			*/
            this.ovos.children[i].position.z = this.ovos.children[j].position.z + (this.ovos.children[j].ovoRadius*2);
          }
        }
      }
    }
  ...
  updateOvos(maxHeight) {
    if ((this.countOvosBu+this.countOvosMa)<this.numOvos) {
      var tipo;

      if (this.countOvosBu >= this.numOvos*0.2) {
        tipo = Ovo.MA;
      } else {
      ...
      switch (tipo) {
        case Ovo.MA:
          var ovo = new Ovo({material:new THREE.MeshPhongMaterial ({color: 0xff0000, specular: 0xfbf804, shininess: 70}), ovoTipo:tipo, posicionZ:this.deep/2});
          ovo.position.y = Math.floor((Math.random() * (maxHeight+ovo.ovoRadius-1))+ovo.ovoRadius);
          ovo.position.x = Math.floor((Math.random() * (this.width+ovo.ovoRadius-1))-(this.width/2+ovo.ovoRadius-1));
          this.ovos.add(ovo);
          this.countOvosMa += 1;
          break;
        case Ovo.BU:
          var ovo = new Ovo({material:new THREE.MeshPhongMaterial ({color: 0x00ff00, specular: 0xfbf804, shininess: 70}), ovoTipo:tipo, posicionZ:this.deep/2});
          ovo.position.y = Math.floor((Math.random() * (maxHeight+ovo.ovoRadius-1))+ovo.ovoRadius);
          ovo.position.x = Math.floor((Math.random() * (this.width+ovo.ovoRadius-1))-(this.width/2+ovo.ovoRadius-1));
          this.ovos.add(ovo);
          this.countOvosBu += 1;
    ...
    if (k=>0) {
      for (var i = k; i < this.ovos.children.length; i++) {
      	this.ovos.children[i].posicionZ -= 0.5;
      	if (this.ovos.children[i].posicionZ <= -(this.deep/2)) {
      		this.ovos.children[i].posicionZ = this.deep/2;
      	}
      	this.ovos.children[i].position.z = this.ovos.children[i].posicionZ;        
      }
    }
    this.updateColisionOvos();
  }
  ...
  /// delete a box on the ground
  /**
   * @param event - Mouse information
   * @param action - Which action is going to be processed: delete it or finish the action.
   */
